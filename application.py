...
def create_signal(create_n_clicks, user_id, signal_id, signal_description, s3):
    ...
    if create_n_clicks != 0:
        res = requests.post("http://127.0.0.1:8080/api/signal/create", headers=header, data=json.dumps(payload))
        res_json = res.json()
...
def modify_signal(modify_n_clicks, user_id, signal_id, signal_description, s3):
    ...
    if modify_n_clicks != 0:
        res = requests.put("http://127.0.0.1:8080/api/signal/modify", headers=header, data=json.dumps(payload))
        res_json = res.json()
...
def read_signal(readit_n_clicks, user_id, signal_id):
    ...
    if readit_n_clicks != 0:
        res = requests.get("http://127.0.0.1:8080/api/signal/read", headers=header, data=json.dumps(payload))
        res_json = res.json()
...
def delete_signal(delete_n_clicks, user_id, signal_id, signal_description):
    ...
    if delete_n_clicks is not None:
        res = requests.delete("http://127.0.0.1:8080/api/signal/modify", headers=header, data=json.dumps(payload))
        res_json = res.json()
